...
def deep_auto_encoder(t_conf):
    ...
    x = Conv2D(filters=8, kernel_size=(3, 3), strides=1, activation='relu', padding='same', name='conv2')(x)
    x = MaxPooling2D(pool_size=(2, 2), strides=2, name='pool2')(x)
    x = Conv2D(filters=8, kernel_size=(3, 3), strides=1, activation='relu', padding='same', name='conv3')(x)
    encoded = MaxPooling2D(pool_size=(2, 2), strides=2, padding='same', name='pool3')(x)

    x = Conv2D(filters=8, kernel_size=(3, 3), strides=1, activation='relu', padding='same', name='conv4')(encoded)
    x = UpSampling2D((2, 2), name='upsa1')(x)
    x = Conv2D(filters=8, kernel_size=(3, 3), strides=1, activation='relu', padding='same', name='conv5')(x)
    x = UpSampling2D((2, 2), name='upsa2')(x)
    x = Conv2D(filters=16, kernel_size=(3, 3), strides=1, activation='relu', name='conv6')(x)
